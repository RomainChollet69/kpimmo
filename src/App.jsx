...
function Auth({ onGuest, guestData }) {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: "https://kpimmo.io" }
    });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#fff", fontFamily: "'DM Sans', sans-serif", padding: "0 20px", display: "flex", flexDirection: "column", width: "100%", maxWidth: 430, margin: "0 auto", justifyContent: "center" }}>
      ...
      <div style={{ animation: "fadeIn 0.5s 0.1s both" }}>
        {/* Google button */}
        <button onClick={handleGoogle} disabled={loading} style={{ width: "100%", padding: 16, borderRadius: 14, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans'", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 20 }}>
      ...
      <div style={{ padding: "16px 20px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.04)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 10, background: "rgba(10,10,15,0.85)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {isGuest ? (
            ...
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowAccount(p => !p)} style={{ width: 36, height: 36, borderRadius: 18, background: showAccount ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : "linear-gradient(135deg, #4f46e5, #7c3aed)", border: "2px solid " + (showAccount ? "#a78bfa" : "rgba(255,255,255,0.15)"), cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "'Outfit'", letterSpacing: -0.5, padding: 0 }}>
